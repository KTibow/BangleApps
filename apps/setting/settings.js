...
function showMainMenu() {
    ...
    const mainmenu = {
        ...
        '< Back': () => load(),
        /*LANG*/
        'Apps': () => showAppSettingsMenu(),
        /*LANG*/
        'System': () => showSystemMenu(),
        /*LANG*/
        'Bluetooth': () => showBLEMenu(),
        /*LANG*/
        'Alerts': () => showAlertsMenu(),
        /*LANG*/
        'Utils': () => showUtilMenu()
    };
        ...
        beepMenuItem = {
            ...
            onchange: v => {
                ...
                if (v == 1) {
                    ...
                    setTimeout(() => D18.reset(), 200);
                } // piezo on Bangle.js 1
                else if (v == 2) {
                    analogWrite(VIBRATE, 0.1, {
                        freq: 2000
                    });
...
function showThemeMenu() {
    ...
    function upd(th) {
        ...
        g._reset = g.reset;
        g.reset = function(n) {
            return g._reset().setColor(th.fg).setBgColor(th.bg);
        };
        g.clear = function(n) {
            if (n) g.reset();
...
function showUtilMenu() {
    var menu = {
        ...
        'Reset Settings': () => {
            E.showPrompt('Reset to Defaults?', {
                title: "Settings"
            }).then((v) => {
                if (v) {
        ...
        /*LANG*/
        'Turn Off': () => {
            if (Bangle.softOff) Bangle.softOff();
            else Bangle.off()
        }
    };
    if (Bangle.factoryReset) {
        menu['Factory Reset'] = () => {
            E.showPrompt('This will remove everything!', {
                title: "Factory Reset"
            }).then((v) => {
                if (v) {
                    E.showMessage();
...
function showAppSettings(app) {
    const showError = msg => {
        ...
        setWatch(showAppSettingsMenu, BTN1, {
            repeat: false
        });
    }
