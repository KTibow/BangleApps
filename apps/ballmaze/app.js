(() => {
    ...
    function playMaze(size) {
         ...
         */
        function findRoute() {
            let dist = new Uint16Array(rooms.length),
                todo = [0];
            dist.fill(-1);
            dist[0] = 0;
            while (true) {
        ...
        function drawMaze() {
            ...
            for (let row = range.top; row <= range.bottom; row++) {
                for (let col = range.left; col <= range.right; col++) {
                    const walls = rooms[row * cols + col],
                        x = col * w,
                        y = row * h;
        ...
        function move() {
            ...
            const fx = (-a.x * w) - (sign(vx) * d * a.z),
                fy = (-a.y * w) - (sign(vy) * d * a.z);
            vx += fx / m;
            vy += fy / m;
         ...
         */
        function bounce() {
            const row = Math.floor(y / sH * rows),
                col = Math.floor(x / sW * cols),
                i = row * cols + col,
                walls = rooms[i];
            const left = col * cW,
                right = (col + 1) * cW,
