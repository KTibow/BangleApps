...
const calen10 = {
    ...
    "dx": -4.5,
    "dy": -4.5
}; // positioning for usual calendar line
const calen7 = {
    "size": 32,
    "pt0": [62 - g_x_off, 16],
    "step": [20, 0],
    "dx": -4.5,
    "dy": -4.5
}; // positioning for S-day calendar line
const time5 = {
    "size": 48,
    "pt0": [64 - g_x_off, 24],
    "step": [30, 0],
    "dx": -6.5,
    "dy": -6.5
}; // positioning for lull time line
const time6 = {
    "size": 48,
    "pt0": [48 - g_x_off, 24],
    "step": [30, 0],
    "dx": -6.5,
    "dy": -6.5
}; // positioning for twinkling time line
...
function writeDozTime(text, def) {
    ...
    for (let i in text) {
        if (text[i] == "a") {
            ...
            g_t.drawString("2", x + def.dx, y + def.dy);
        } else if (text[i] == "b") {
            g_t.setFontAlign(0, 0, 2);
            g_t.drawString("3", x + def.dx, y + def.dy);
        } else {
...
function writeDozDate(text, def, colour) {
    ...
    for (let i in text) {
        if (text[i] == "a") {
            ...
            g_d.drawString("2", x + def.dx, y + def.dy);
        } else if (text[i] == "b") {
            g_d.setFontAlign(0, 0, 2);
            g_d.drawString("3", x + def.dx, y + def.dy);
        } else {
