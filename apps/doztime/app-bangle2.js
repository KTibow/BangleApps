...
const calen10 = {
    ...
    "dx": -4.5,
    "dy": -4.5
}; // positioning for usual calendar line ft w 32, 32-g, step 20
const calen7 = {
    "size": 26,
    "pt0": [48 - g_x_off, 16],
    "step": [16, 0],
    "dx": -4.5,
    "dy": -4.5
}; // positioning for S-day calendar line ft w 32, 62-g, step 20
const time5 = {
    "size": 42,
    "pt0": [39 - g_x_off, 24],
    "step": [26, 0],
    "dx": -6.5,
    "dy": -6.5
}; // positioning for lull time line ft w 48, 64-g, step 30
const time6 = {
    "size": 42,
    "pt0": [26 - g_x_off, 24],
    "step": [26, 0],
    "dx": -6.5,
    "dy": -6.5
}; // positioning for twinkling time line ft w 48, 48-g, step 30
...
var g_d = Graphics.createArrayBuffer(g_width, g_height_d, 1, {
    'msb': true
});
var g_t = Graphics.createArrayBuffer(g_width, g_height_t, 1, {
    'msb': true
});
...
function writeDozTime(text, def) {
    ...
    for (let i in text) {
        if (text[i] == "a") {
            g_t.setFontAlign(0, 0, 2);
            g_t.drawString("2", x + 2 + def.dx, y + 1 + def.dy);
        } //+1s are new
        else if (text[i] == "b") {
            g_t.setFontAlign(0, 0, 2);
            g_t.drawString("3", x + 2 + def.dx, y + 1 + def.dy);
        } //+1s are new
        else {
            g_t.setFontAlign(0, 0, 0);
            g_t.drawString(text[i], x, y);
        }
        x = x + def.step[0];
        y = y + def.step[1];
    }
...
function writeDozDate(text, def, colour) {
    ...
    for (let i in text) {
        if (text[i] == "a") {
            g_d.setFontAlign(0, 0, 2);
            g_d.drawString("2", x + 2 + def.dx, y + 1 + def.dy);
        } //+1s new
        else if (text[i] == "b") {
            g_d.setFontAlign(0, 0, 2);
            g_d.drawString("3", x + 2 + def.dx, y + 1 + def.dy);
        } //+1s new
        else {
            g_d.setFontAlign(0, 0, 0);
            g_d.drawString(text[i], x, y);
        }
        x = x + def.step[0];
        y = y + def.step[1];
    }
...
function drawTime() {
    ...
    if (lastX > x) {
        res = getDate(dt);
    } // calculate date once at start-up and once when turning over to a new day
