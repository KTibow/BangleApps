...
function viewTracks() {
    ...
    var found = false;
    require("Storage").list(/^recorder\.log.*\.csv$/, {
        sf: true
    }).forEach(filename => {
        found = true;
...
function getTrackInfo(filename) {
    ...
    while (l !== undefined) {
        ...
        n = +c[lonIdx];
        if (n > maxLong) maxLong = n;
        if (n < minLong) minLong = n;
    }
...
function plotTrack(info) {
    ...
    if (info.qOSTM) {
        ...
        getMapXY = function(lat, lon) {
            "ram"
            return {
                x: cx + Math.round((long - info.lon) * info.lfactor * info.scale),
