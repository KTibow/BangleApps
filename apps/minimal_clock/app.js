  ...
  function updateClockFaceSize() {
      ...
      x = WidgetLayouts.tl.x;
      y = WidgetLayouts.tl.y + 24;
      dx = x - cx;
      dy = y - cy;
      if (dx * dx + dy * dy < r2) {
          cy = CenterY + 12;
      ...
      x = WidgetLayouts.tr.x;
      y = WidgetLayouts.tr.y + 24;
      dx = x - cx;
      dy = y - cy;
      if (dx * dx + dy * dy < r2) {
          cy = CenterY + 12;
      ...
      x = WidgetLayouts.bl.x;
      y = WidgetLayouts.bl.y;
      dx = x - cx;
      dy = y - cy;
      if (dx * dx + dy * dy < r2) {
          cy = CenterY - 12;
      ...
      x = WidgetLayouts.br.x;
      y = WidgetLayouts.br.y;
      dx = x - cx;
      dy = y - cy;
      if (dx * dx + dy * dy < r2) {
          cy = CenterY - 12;
  ...
  for (let i = 0, l = HandPolygon.length; i < l; i += 2) {
      ...
      if (i < l / 2) {
          HourHandPolygon[i + 1] -= HourHandLength;
      }
      if (i > l / 2) {
          HourHandPolygon[i + 1] += HandOffset;
      }
  }
  ...
  for (let i = 0, l = HandPolygon.length; i < l; i += 2) {
      ...
      if (i < l / 2) {
          MinuteHandPolygon[i + 1] -= MinuteHandLength;
      }
      if (i > l / 2) {
          MinuteHandPolygon[i + 1] += HandOffset;
      }
  }
  ...
  function drawClockHands() {
      ...
      let sPhi = Math.sin(SecondsAngle),
          cPhi = Math.cos(SecondsAngle);
