  ...
  let normalizedColorSet = {
      ...
      green: g.toColor(0, 1, 0),
      magenta: g.toColor(1, 0, 1),
      blue: g.toColor(0, 0, 1),
  ...
  function Label(Text, Options) {
      function renderLabel(Details) {
          let halfWidth = Details.w / 2,
              xAlignment = Details.halign || 0;
          let halfHeight = Details.h / 2,
              yAlignment = Details.valign || 0;
          let Padding = Details.pad || 0;

          g.setColor(Details.col || g.theme.fg || '#000000');

          if (Details.font != null) {
              g.setFont(Details.font);
          }
  ...
  if (g.drawRoundedRect == null) {
      g.drawRoundedRect = function drawRoundedRect(x1, y1, x2, y2, r) {
          ...
          while (y <= x) {
              ...
              this.setPixel(cx1 - x, cy1 - y);
              this.setPixel(cx1 - y, cy1 - x);
              this.setPixel(cx2 + x, cy1 - y);
              this.setPixel(cx2 + y, cy1 - x);
              this.setPixel(cx2 + x, cy2 + y);
              this.setPixel(cx2 + y, cy2 + x);
              this.setPixel(cx1 - x, cy2 + y);
              this.setPixel(cx1 - y, cy2 + x);
          }
      };
  }
  ...
  function EventConsumerAtPoint(HandlerName, x, y) {
      ...
      function ConsumerIn(Control) {
          ...
          if (typeof Control[HandlerName] === 'function') {
              return Control;
          }
  ...
  let legible = Object.assign({
      col: '#000000',
      bgCol: '#FFFFFF'
  }, StdFont);
  let leftAligned = Object.assign({
      halign: -1,
      valign: 0
  }, legible);
  let MainLabel = Object.assign({
      pad: 4,
      width: leftColumnWidth
  }, leftAligned);
  let halfWidthButton = Object.assign({
      pad: 4,
      width: halfWidth
  }, legible);
  ...
  ScreenSet['ColorSelectionScreen'] = new Layout({
      ...
      c: [
          ...
          }),
          {
              type: 'h',
              c: [
                  ColorSelectionView('#000000', {
                      common: StdSelectionView,
                      id: 'black',
                      onTouch: () => selectColor(0, 0, 0)
                  }),
                  ColorSelectionView('#FF0000', {
                      common: StdSelectionView,
                      id: 'red',
                      onTouch: () => selectColor(1, 0, 0)
                  }),
                  ColorSelectionView('#00FF00', {
                      common: StdSelectionView,
                      id: 'green',
                      onTouch: () => selectColor(0, 1, 0)
                  }),
                  ColorSelectionView('#0000FF', {
                      common: StdSelectionView,
                      id: 'blue',
          ...
          },
          {
              type: 'h',
              c: [
                  ColorSelectionView('#FFFFFF', {
                      common: StdSelectionView,
                      id: 'white',
                      onTouch: () => selectColor(1, 1, 1)
                  }),
                  ColorSelectionView('#FFFF00', {
                      common: StdSelectionView,
                      id: 'yellow',
                      onTouch: () => selectColor(1, 1, 0)
                  }),
                  ColorSelectionView('#FF00FF', {
                      common: StdSelectionView,
                      id: 'magenta',
                      onTouch: () => selectColor(1, 0, 1)
                  }),
                  ColorSelectionView('#00FFFF', {
                      common: StdSelectionView,
                      id: 'cyan',
  ...
  function gotoScreen(ScreenName) {
      ...
      g.setColor('#000000');
      g.setBgColor('#FFFFFF'); // assert legibility
      g.clear();
