...
function setButtons() {
    setWatch(buttonPressed.bind(null, 1), BTN1, {
        repeat: true,
        edge: "rising"
    });
    setWatch(buttonPressed.bind(null, 2), BTN2, {
        repeat: true,
        edge: "rising"
    });
    setWatch(nextFace, BTN3, {
        repeat: true,
        edge: "rising"
    });

    setWatch(buttonReleased.bind(null, 1), BTN1, {
        repeat: true,
        edge: "falling"
    });
    setWatch(buttonReleased.bind(null, 2), BTN2, {
        repeat: true,
        edge: "falling"
    });
...
GPS.prototype.markWaypoint = function() {
    ...
    if (this.waypointHasLocation()) {
        waypoints[this.wp_index] = {
            name: this.wp_current.name,
            lat: 0,
            lon: 0
        };
    } else {
        waypoints[this.wp_index] = {
            name: this.wp_current.name,
            lat: this.last_fix.lat,
            lon: this.last_fix.lon
        };
    }
...
OsGridRef.latLongToOsGrid = function(point) {
    ...
    var lat0 = (49).toRad(),
        lon0 = (-2).toRad(); // NatGrid true origin is 49Ã¯Â¿Â½N,2Ã¯Â¿Â½W
    var N0 = -100000,
