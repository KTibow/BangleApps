...
let buf = Graphics.createArrayBuffer(screenWidth, screenHeight, 1, {
    msb: true
});
...
function renderSecText() {
    ...
    fillLinesLaterGens();
    g.drawImage({
            width: textWidthScreenSec,
            height: textHeightScreen,
            bpp: 1,
            transparent: 0,
            buffer: textbufSec.buffer
        },
...
function flip(dontclear) {
    ...
    if (generation < genEnd) {
        ...
        g.drawImage({
                width: textWidthScreenMin,
                height: textHeightScreen,
                bpp: 1,
                buffer: textbufMin.buffer
            },
...
function updateMinuteText(date) {
    ...
    g.drawImage({
            width: textWidthScreen - textWidthScreenSec,
            height: textHeightScreen,
            bpp: 1,
            transparent: 0,
            buffer: textbufMin.buffer
...
function nextLineComp() {
    ...
    for (let x = 1; x < noOfSquX + 1; x++) {
        if (skipCell(x - 1, y - 1)) {
            break;
        } // the rest of this line would be skipped, too
        let ind = x + y * (noOfSquX + 2);
