...
function getApps() {
    ...
    const raw_apps = Storage.list(/\.info$/).filter(app => app.endsWith('.info')).map(app => Storage.readJSON(app, 1) || {
            name: "DEAD: " + app.substr(1)
        })
    ...
    visibleApps.forEach(app => {
        ...
        if (icon) {
            ...
            try {
                ...
                g.drawImage(icon, x - rescale, y - rescale, {
                    scale: imageScale
                });
