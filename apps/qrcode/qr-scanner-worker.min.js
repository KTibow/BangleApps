...
(function() {
    function T(a, b) {
        ...
        try {
            d += decodeURIComponent(c.map(a => `%${("0"+a.toString(16)).substr(-2)}`).join(""))
        } catch (e) {}
        return {
            bytes: c,
            text: d
    ...
    function U(a, b) {
        ...
        for (b = {
            ...
            if (d === t.ECI) 0 === a.readBits(1) ? b.chunks.push({
                    type: r.ECI,
                    assignmentNumber: a.readBits(7)
                }) :
                0 === a.readBits(1) ? b.chunks.push({
                    type: r.ECI,
                ...
                d = [];
                f = "";
                for (g = e.readBits([9, 11, 13][c]); 2 <= g;) n = e.readBits(11), k = Math.floor(n / 45), n %= 45, d.push(B[k].charCodeAt(0), B[n].charCodeAt(0)),
                    f += B[k] + B[n], g -= 2;
                1 === g && (e = e.readBits(6), d.push(B[e].charCodeAt(0)), f += B[e]);
                d = {
                    bytes: d,
                    text: f
                };
            ...
            } else if (d === t.Byte) d = T(a, c), b.text += d.text, b.bytes.push(...d.bytes), b.chunks.push({
                type: r.Byte,
                bytes: d.bytes,
                text: d.text
            });
                ...
                for (g = 0; g < e; g++) k = f.readBits(13), k = Math.floor(k / 192) << 8 | k % 192, k = 7936 > k ? k + 33088 : k + 49472, d.push(k >> 8, k & 255);
            ...
            } else d === t.StructuredAppend && b.chunks.push({
    ...
    function X(a, b) {
        ...
        if (1 === g) b = [b.getCoefficient(1)];
            ...
            e = Array(g);
            f = 0;
            for (h = 1; h < a.size && f < g; h++) 0 === b.evaluateAt(h) && (e[f] = a.inverse(h), f++);
            b = f !== g ? null : e
        }
        ...
        for (g = 0; g < e; g++) {
            ...
            let c = 1;
            for (let d =
                    0; d < e; d++) g !== d && (c = a.multiply(c, J(1, a.multiply(b[d], h))));
    ...
    function Z(a, b, c) {
        ...
        for (var g of b.alignmentPatternCenters)
            for (var h of b.alignmentPatternCenters) 6 ===
                g && 6 === h || 6 === g && h === e - 7 || g === e - 7 && 6 === h || f.setRegion(g - 2, h - 2, 5, 5, !0);
        f.setRegion(6, 9, 1, e - 17, !0);
        f.setRegion(9, 6, e - 17, 1, !0);
        6 < b.versionNumber && (f.setRegion(e - 11, 0, 3, 6, !0), f.setRegion(0, e - 11, 6, 3, !0));
        b = f;
        g = [];
        e = h = 0;
        f = !0;
    ...
    function ea(a, b, c) {
        ...
        d.ecBlocks.forEach(a => {
            for (let b = 0; b < a.numBlocks; b++) e.push({
        ...
        for (c = 0; c < b; c++)
            for (var g of e) g.codewords.push(a.shift());
        if (1 < d.ecBlocks.length)
            for (g = d.ecBlocks[0].numBlocks, b = d.ecBlocks[1].numBlocks, c = 0; c < b; c++) e[g + c].codewords.push(a.shift());
        for (; 0 < a.length;)
            for (let b of e) b.codewords.push(a.shift());
        return e
    }
    ...
    function L(a) {
        ...
        for (let b of d) {
            ...
            if (!d) return null;
            for (let e =
                    0; e < b.numDataCodewords; e++) c[a++] = d[e]
        }
        try {
    ...
    function ha(a, b) {
        ...
        for (let e = 0; e < b.dimension; e++)
            for (let f = 0; f < b.dimension; f++) {
                let b = d(f + .5, e + .5);
                c.set(f, e, a.get(Math.floor(b.x),
                    Math.floor(b.y)))
            }
        return {
            matrix: c,
    ...
    function ia(a, b, c) {
        let d = y(a, b),
            ...
            f = y(a, c),
            g, h, k;
        e >= d && e >= f ? [g, h, k] = [b, a, c] : f >= e && f >= d ? [g, h, k] = [a, b, c] : [g, h, k] = [a, c, b];
        0 > (k.x - h.x) * (g.y - h.y) - (k.y - h.y) * (g.x - h.x) && ([g, k] = [k, g]);
        return {
            bottomLeft: g,
            topLeft: h,
            topRight: k
    ...
    function ja(a, b, c, d) {
        d = (x(z(a, c, d, 5)) / 7 + x(z(a, b, d, 5)) / 7 + x(z(c, a, d, 5)) / 7 + x(z(b, a, d, 5)) / 7) / 4;
    ...
    function N(a, b, c, d) {
        ...
        if (f) {
            ...
            a = Math.floor(b.y);
            b = Math.floor(b.x)
        } else g = Math.floor(a.x), h = Math.floor(a.y), a = Math.floor(b.x), b = Math.floor(b.y);
        let k = Math.abs(a - g),
            n = Math.abs(b - h),
            m = Math.floor(-k / 2),
            l = g < a ? 1 : -1,
        ...
        c = [];
        for (f = 0; f < d; f++) e[f] && e[f + 1] ? c.push(y(e[f], e[f + 1])) : c.push(0);
        return c
    }
    ...
    function O(a, b, c) {
        try {
            ...
            return Math.sqrt(h.error * h.error + k.error * k.error + n.error * n.error + m.error * m.error) + (Math.pow(h.averageSize - l, 2) + Math.pow(k.averageSize - l, 2) + Math.pow(n.averageSize - l, 2) + Math.pow(m.averageSize - l, 2)) / l
        } catch (d) {
            return Infinity
        }
    }
    ...
    function ka(a) {
        ...
        for (let m = 0; m <= a.height; m++) {
            ...
            for (let b = -1; b <= a.width; b++) {
                ...
                if (h === g) f++;
                    ...
                    g = h;
                    var k = x(l) / 7;
                    k = Math.abs(l[0] - k) < k && Math.abs(l[1] -
                        k) < k && Math.abs(l[2] - 3 * k) < 3 * k && Math.abs(l[3] - k) < k && Math.abs(l[4] - k) < k && !h;
                    var n = x(l.slice(-3)) / 3;
                    h = Math.abs(l[2] - n) < n && Math.abs(l[3] - n) < n && Math.abs(l[4] - n) < n && h;
                    if (k) {
                        let a = b - l[3] - l[4],
                            d = a - l[2];
                        k = {
                        ...
                        n = c.filter(b => d >= b.bottom.startX && d <= b.bottom.endX || a >= b.bottom.startX && d <= b.bottom.endX || d <= b.bottom.startX && a >= b.bottom.endX && 1.5 > l[2] / (b.bottom.endX - b.bottom.startX) && .5 < l[2] / (b.bottom.endX - b.bottom.startX));
                        0 < n.length ? n[0].bottom = k : c.push({
                            top: k,
                            bottom: k
                        })
                    }
                    if (h) {
                        ...
                        k = e.filter(b => c >= b.bottom.startX && c <= b.bottom.endX || a >= b.bottom.startX && c <= b.bottom.endX || c <= b.bottom.startX && a >= b.bottom.endX && 1.5 > l[2] / (b.bottom.endX - b.bottom.startX) && .5 < l[2] / (b.bottom.endX - b.bottom.startX));
                        0 < k.length ? k[0].bottom = h : e.push({
                            top: h,
                            bottom: h
                        })
                    }
        ...
        d.push(...e);
        c = [];
        for (var m of b) 2 > m.bottom.y - m.top.y || (b = (m.top.startX + m.top.endX + m.bottom.startX + m.bottom.endX) / 4, e = (m.top.y + m.bottom.y + 1) / 2, a.get(Math.round(b), Math.round(e)) && (f = [m.top.endX - m.top.startX, m.bottom.endX - m.bottom.startX, m.bottom.y - m.top.y + 1], f = x(f) / f.length, g = O({
            x: Math.round(b),
            y: Math.round(e)
        }, [1, 1, 3, 1, 1], a), c.push({
            score: g,
        ...
        for (b = 0; b < Math.min(c.length, 5); ++b) {
            e = c[b];
            f = [];
            for (var l of c) l !== e && f.push(Object.assign(Object.assign({},
                l), {
        ...
        m && l.push({
            alignmentPattern: {
                x: m.alignmentPattern.x,
                y: m.alignmentPattern.y
            },
            bottomLeft: {
                x: v.x,
                y: v.y
    ...
    function P(a, b, c, d, e) {
        ...
        b = b.map(b => {
            const c = (b.top.startX + b.top.endX + b.bottom.startX + b.bottom.endX) / 4;
        ...
        return {
            alignmentPattern: 15 <= c && b.length ? b[0] : n,
            dimension: f
        }
    }
        ...
        for (let e of b) {
            ...
            if (null == c) c = null;
                ...
                if (d) c = d;
                    ...
                    for (d = 0; d < c.width; d++)
                        for (let a = d + 1; a < c.height; a++) c.get(d, a) !== c.get(a, d) && (c.set(d, a, !c.get(d, a)), c.set(a, d, !c.get(a, d)));
                    c = L(c)
                }
            }
            if (c) return {
    ...
    function I(a, b, c, d = {}) {
        ...
        if (g.useIntegerApproximation)
            for (var l = 0; l < c; l++)
                for (var p = 0; p < b; p++) {
                    var q = 4 * (l * b + p);
                    m.set(p, l, g.red * a[q] + g.green * a[q + 1] + g.blue * a[q + 2] + 128 >> 8)
                } else
                    for (l = 0; l < c; l++)
                        for (p = 0; p < b; p++) q = 4 *
        ...
        for (p = 0; p < l; p++)
            for (q = 0; q < g; q++) {
                ...
                24 >= r - u && (t = u / 2, 0 < p && 0 < q && (r = (v.get(q, p - 1) + 2 * v.get(q - 1, p) + v.get(q - 1, p - 1)) / 4, u < r && (t = r)));
                v.set(q, p, t)
            }
        h ? (p = new Uint8ClampedArray(a.buffer, n, k), n += k, p = new A(p,
            b)) : p = A.createEmpty(b, c);
        q = null;
        f && (h ? (a = new Uint8ClampedArray(a.buffer, n, k), q = new A(a, b)) : q = A.createEmpty(b, c));
        for (b = 0; b < l; b++)
            for (a = 0; a < g; a++) {
                c = g - 3;
                c = 2 > a ? 2 : a > c ? c : a;
                ...
                c = k / 25;
                for (h = 0; 8 > h; h++)
                    for (k = 0; 8 > k; k++) n = 8 * a + h, u = 8 * b + k, r = m.get(n, u), p.set(n, u, r <= c), f && q.set(n, u, !(r <= c))
            }
        f = f ? {
            binarized: p,
            inverted: q
        ...
        (f = Q(d ? x : w)) || "attemptBoth" !== e.inversionAttempts && "invertFirst" !== e.inversionAttempts ||
            (f = Q(d ? w : x));
        return f
    }
    class A {
    ...
    class V {
        ...
        readBits(a) {
            ...
            if (0 < a) {
                for (; 8 <= a;) b = b << 8 | this.bytes[this.byteOffset] & 255, this.byteOffset++,
                    a -= 8;
    ...
    class w {
        constructor(a, b) {
            ...
            if (1 < c && 0 === b[0]) {
                ...
                if (d === c) this.coefficients = a.zero.coefficients;
                    ...
                    for (this.coefficients = new Uint8ClampedArray(c - d), a = 0; a < this.coefficients.length; a++) this.coefficients[a] = b[d + a]
        ...
        multiply(a) {
            ...
            for (let d = 0; d < b; d++) c[d] = this.field.multiply(this.coefficients[d], a);
            return new w(this.field, c)
        }
        multiplyPoly(a) {
        ...
        multiplyByMonomial(a, b) {
            ...
            for (let d = 0; d < c; d++) a[d] = this.field.multiply(this.coefficients[d], b);
        ...
        evaluateAt(a) {
            ...
            let c = this.coefficients.length;
            if (1 === a) return this.coefficients.forEach(a => {
                b ^= a
            }), b;
            b = this.coefficients[0];
            for (let d = 1; d < c; d++) b = J(this.field.multiply(a, b), this.coefficients[d]);
            return b
        }
    }
    class Y {
        constructor(a, b, c) {
            ...
            a = 1;
            for (b = 0; b < this.size; b++) this.expTable[b] = a, a *= 2, a >= this.size && (a = (a ^ this.primitive) & this.size - 1);
    ...
    let K = [{
        ...
        aa = [a => 0 === (a.y + a.x) % 2, a => 0 === a.y % 2, a => 0 === a.x % 3, a => 0 === (a.y + a.x) % 3, a => 0 === (Math.floor(a.y / 2) + Math.floor(a.x / 3)) % 2, a => 0 === a.x * a.y % 2 + a.x * a.y % 3, a => 0 === (a.y * a.x % 2 + a.y * a.x % 3) % 2, a => 0 === ((a.y + a.x) % 2 + a.y * a.x % 3) % 2],
