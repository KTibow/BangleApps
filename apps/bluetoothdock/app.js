...
var scanHandlers = [{
    ...
    handler: function(device) {
        ...
        if (frame & 64) { // event
            ...
            if (!deviceInfo[device.id]) deviceInfo[device.id] = {
                id: device.id
            };
            ...
            switch (code) {
                case 0x1004:
                    ...
                    break;
                case 0x1006:
                    event.humidity = d.getInt16(offset + 3) / 10;
                    break;
                case 0x100D:
                    event.temperature = d.getInt16(offset + 3, true) / 10;
                    event.humidity = d.getInt16(offset + 5) / 10;
                    break;
                case 0x1008:
                    event.moisture = d.getUint8(offset + 3);
                    break;
                case 0x1009:
                    event.fertility = d.getUint16(offset + 3, true) / 10;
                    break;
    ...
    handler: function(device) {
        ...
        var j;
        try {
            j = JSON.parse(E.toString(device.manufacturerData));
        } catch (e) {
            return;
        } // not JSON
        if (!deviceInfo[device.id]) deviceInfo[device.id] = {
...
NRF.setScan(parseDevice, {
    filters: scanHandlers.map(s => s.filter),
    timeout: 2000
});
