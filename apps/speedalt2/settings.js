(function(back) {
    ...
    const appMenu = {
        ...
        '< Back': back,
        '< Load GPS Adv Sport': () => {
            load('speedalt2.app.js');
        },
        'Units': function() {
            E.showMenu(unitsMenu);
        },
        'Colours': function() {
            E.showMenu(colMenu);
        },
    ...
    const unitsMenu = {
        ...
        'Mph (spd)': function() {
            setUnits(1.60934, 'mph');
        },
        'm/s (spd)': function() {
            setUnits(3.6, 'm/s');
        },
        'Km (dist)': function() {
            setUnitsDist(1000, 'km');
        },
        'Miles (dist)': function() {
            setUnitsDist(1609.344, 'mi');
        },
        'Nm (dist)': function() {
            setUnitsDist(1852.001, 'nm');
        },
        'Meters (alt)': function() {
            setUnitsAlt(1, 'm');
        },
        'Feet (alt)': function() {
            setUnitsAlt(0.3048, 'ft');
        }
    };
    ...
    const kalMenu = {
        ...
        'Speed': {
            ...
            format: v => v ? "On" : "Off",
            onchange: () => {
                settings.spdFilt = !settings.spdFilt;
                writeSettings();
            }
        },
        'Altitude': {
            ...
            format: v => v ? "On" : "Off",
            onchange: () => {
                settings.altFilt = !settings.altFilt;
                writeSettings();
            }
        }
    };
