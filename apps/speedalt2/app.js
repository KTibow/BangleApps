...
var buf = Graphics.createArrayBuffer(240, 160, 2, {
    msb: true
});
...
let LED = // LED as minimal and only definition (as instance / singleton)
    {
        ...
        write: function(v) {
                this.set(v);
            } // turn on w/ no arg or truey, else off
            ,
        toggle: function() {
            this.set(!this.isOn);
        } // toggle the LED
...
function loadWp() {
    var w = require("Storage").readJSON('waypoints.json') || [{
        name: "NONE"
    }];
...
function stopDraw() {
    canDraw = false;
    if (!tmrLP) tmrLP = setInterval(function() {
...
function setLpMode(m) {
    if (tmrLP) {
        clearInterval(tmrLP);
        tmrLP = false;
    } // Stop any scheduled drop to low power
    if (!gpssetup) return;
    gpssetup.setPowerMode({
        power_mode: m
    });
}
...
if (cfg.spdFilt) var spdFilter = new KalmanFilter({
    R: 0.1,
    Q: 1
});
if (cfg.altFilt) var altFilter = new KalmanFilter({
    R: 0.01,
    Q: 2
});
